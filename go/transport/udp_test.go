...
// TestUDP_WithConn tests UDP transport with conn.Dial for handshake
func TestUDP_WithConn(t *testing.T) {
	...
	// Server goroutine - accept handshake using Listener
	var serverConn *conn.Conn
	go func() {
		// Create listener
		listener, err := conn.NewListener(conn.ListenerConfig{
			LocalKey:  serverKey,
		...
		if serverConn.State() != conn.ConnStateEstablished {
			errCh <- err
			return
		}
	...
	// Client - initiate handshake using Dial
	go func() {
		clientConn, err := conn.Dial(clientTransport, clientTransport.RemoteAddr(), serverKey.Public, clientKey)
		if err != nil {
			errCh <- err
			return
		}
		...
		// Verify connection established
		if clientConn.State() != conn.ConnStateEstablished {
