...
test "relay chain with noise session A->B->C" {
    ...
    const nonce = try session_a.encrypt(payload_enc, &cipher_buf, now_ms);
    ...
    const dec_now_ms: u64 = @intCast(std.time.milliTimestamp());
    const pt_len = try session_c.decrypt(inner_msg.ciphertext, inner_msg.counter, &decrypt_buf, dec_now_ms);
...
test "relay multi-hop with noise session A->B->C->D" {
    ...
    var decrypt_buf: [1024]u8 = undefined;
    const dec2_now_ms: u64 = @intCast(std.time.milliTimestamp());
    const pt_len = try session_d.decrypt(inner_msg.ciphertext, inner_msg.counter, &decrypt_buf, dec2_now_ms);
    const plaintext = decrypt_buf[0..pt_len];
