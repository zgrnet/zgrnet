...
/// A connection to a remote peer, generic over Crypto and Runtime.
///
...
pub fn Conn(comptime Crypto: type, comptime Rt: type) type {
    ...
    return struct {
        ...
        inbound_signal: Rt.Condition = Rt.Condition.init(),
        ...
        pub fn init(allocator: mem.Allocator, cfg: ConnConfig) Self {
            return Self{
                .allocator = allocator,
        ...
        pub fn accept(self: *Self, msg: *const HandshakeInit) ConnError![message.handshake_resp_size]u8 {
            self.mutex.lock();
        ...
        fn completeHandshakeLocked(self: *Self, hs: *const HandshakeState, remote_idx: u32, remote_pk: ?Key) ConnError!void {
            if (!hs.isFinished()) {
        ...
        pub fn sendKeepalive(self: *Self) ConnError!void {
        ...
        pub fn recv(self: *Self, out_buf: []u8) ConnError!RecvResult {
            ...
            const tmsg = message.parseTransportMessage(buf[0..result.bytes_read]) catch return ConnError.MessageError;

            // Verify receiver index
            if (tmsg.receiver_index != self.local_idx) {
                return ConnError.InvalidReceiverIndex;
            }

            // Decrypt
            const plaintext_len = tmsg.ciphertext.len - session_mod.tag_size;
        ...
        pub fn getRemotePublicKey(self: *Self) Key {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn setRemoteAddr(self: *Self, addr: Addr) void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn hasInbound(self: *Self) bool {
            self.mutex.lock();
            defer self.mutex.unlock();
...
const TestConn = Conn(StdCrypto, StdRt);
...
test "conn handshake and communication" {
    ...
    const initiator_key = TestKeyPair.generate();
    const responder_key = TestKeyPair.generate();
    ...
    var responder = TestConn.init(allocator, .{
    ...
    const responder_thread = try Thread.spawn(.{}, struct {
        fn run(r: *TestConn, rt: *transport_mod.MockTransport) void {
    ...
    const initiator = dial_mod.dial(StdCrypto, StdRt, .{
        .allocator = allocator,
...
test "tick conn timeout" {
    ...
    const session = TestSession.init(.{
        .local_index = 1,
        .remote_index = 2,
